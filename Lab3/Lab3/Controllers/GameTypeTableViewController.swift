...
class GameTypeTableViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "releaseSegue" {
            if let gameController = segue.destination as? ReleasesTableViewController {
                if let path = tableView.indexPath(for: (sender as? UITableViewCell)!) {
                    gameController.title = categories[path.row]
                    gameController.selectedCategory = path.row
                    gameController.loader = dataLoader
                }
            }
        }
    }
